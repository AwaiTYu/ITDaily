package com.json.itdaily.utils;
...
public class AutoUtils {
    ...
    public static void auto(View view) {
        if (view.getTag() != null && view.getTag() instanceof Integer && (Integer) view.getTag() == -1000)
            return;
    ...
    public static void autoSize(View view) {
        ...
        if (lp == null) return;
        if (view instanceof ImageView && lp.width > 0 && lp.height > 0) {
            float rate = 1.0f;
            rate = lp.height * 1.0f / lp.width;
            lp.width = getDisplayWidthValue(lp.width);
            lp.height = (int) (rate * lp.width);
        } else {
            if (lp.width > 0) {
                lp.width = getDisplayWidthValue(lp.width);
            }
    ...
    public static float getTextSize(float textPixelsSize) {
        return (float) (textPixelsRate * textPixelsSize);
    }
}
